...
public class DataBaseUserInMemory implements UserDB {

    private final ArrList<User> list;
    private static DataBaseUserInMemory instance;

    private DataBaseUserInMemory() {
        list = new ArrList<>();
    }

    public static DataBaseUserInMemory getInstance() {
        if (instance == null) {
            instance = new DataBaseUserInMemory();
        }
        return instance;
